...
function ProductCard({ p }: { p: Producto }) {
  ...
  return (
    <div className='bg-white rounded-2xl border border-[#f0e6d3] overflow-hidden hover:-translate-y-1.5 hover:shadow-lg transition-all duration-300'>
      <div className={`h-40 bg-gradient-to-br ${bg} flex items-center justify-center relative text-6xl`}>
        {p.emoji}
        {p.badge && (
  ...
  return (
    <section id='productos' className='py-24 px-6 lg:px-16 max-w-7xl mx-auto'>
        ...
        {tabs.map((t) => {
          ...
          return (
            ...
            >
              {t.label}
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.value ? 'bg-white/20' : 'bg-[#f0e6d3]'}`}
              >
                {count}
              </span>
            </button>
      ...
      {loading ? (
        ...
        <div className='text-center py-20 text-[#8a7060] text-sm'>No hay productos disponibles en esta categoría</div>
      ) : tab === 'alfajor' ? (
